...
!=----------------------------------------------------------------------------=!
     ...
     !$omp declare target(g, gl, igtongl)
     INTEGER :: ngm = 0
   ...
   CONTAINS
     ...
     SUBROUTINE gvect_init( ngm_ , comm )
       ...
       !$omp target enter data map(alloc:mill, g)
       !
       RETURN
       !
     ...
     SUBROUTINE deallocate_gvect(vc)
       ...
       !$omp target exit data map(delete:eigts1,  eigts2,  eigts3)
       !$omp target exit data map(delete:mill, g)
       !
